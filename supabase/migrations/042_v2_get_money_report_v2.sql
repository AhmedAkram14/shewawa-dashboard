-- Migration 042: get_money_report v2 — full two-sided cash flow
-- Customer side: revenue expected, deposits collected, outstanding from customers.
-- Factory side:  cost agreed (unit_cost × qty), payments made, owed to factories.
-- Profit:        expected gross profit and margin.
...
BEGIN
  ...
  -- ── Customer side: active orders (not delivered, not cancelled) ───────────
  active_orders AS (
    SELECT
      o.id,
      o.order_number,
  ...
  -- Only lines with unit_cost set contribute to cost_agreed.
  -- Lines with NULL unit_cost are counted separately as cost_lines_unknown.
  ...
  SELECT json_build_object(
    ...
    'active_order_count',    cs.order_count,
    'by_status',             COALESCE(
  ...
  FROM customer_summary cs, factory_cost_summary fcs, factory_payment_summary fps;
