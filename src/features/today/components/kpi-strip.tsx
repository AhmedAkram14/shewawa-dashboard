...
function InteractiveChip({
  ...
  return (
    <button
      ...
      className="flex flex-1 flex-col items-center gap-0.5 rounded-lg border bg-card px-2 py-2.5 transition-colors hover:bg-accent active:bg-accent/80"
    >
...
function StaticChip({ value, label }: { value: number; label: string }) {
  return (
    <div className="flex flex-1 flex-col items-center gap-0.5 rounded-lg border border-dashed bg-card px-2 py-2.5 opacity-70">
      <span className="text-xl font-semibold tabular-nums">{value}</span>
      <span className="text-center text-[11px] leading-tight text-muted-foreground">
        {label}
      </span>
